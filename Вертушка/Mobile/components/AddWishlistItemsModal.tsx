/**
 * AddWishlistItemsModal — выбор пластинок из вишлиста для добавления в папку
 * Показывает все WishlistItem'ы, уже добавленные — с галочкой и неактивны
 */
...
interface AddWishlistItemsModalProps {
  visible: boolean;
  onClose: () => void;
  existingWishlistItemIds: Set<string>;
  onAdd: (wishlistItemIds: string[]) => Promise<void>;
}

export function AddWishlistItemsModal({
  visible,
  onClose,
  existingWishlistItemIds,
  onAdd,
}: AddWishlistItemsModalProps) {
  const { wishlistItems, fetchWishlistItems, isLoading } = useCollectionStore();
  ...
  const renderItem = ({ item }: { item: WishlistItem }) => {
    const isInFolder = existingWishlistItemIds.has(item.id);
      ...
      <View style={[styles.container, { paddingTop: insets.top }]}>
        ...
        {isLoading ? (
          ...
          <View style={styles.centered}>
            ...
            <Text style={styles.emptyText}>Вишлист пуст</Text>
          </View>
        ) : (
          <FlatList
            data={wishlistItems}
            ...
            extraData={existingWishlistItemIds}
