...
**Архитектура**: per-shop парсер (наследник `BaseStoreParser`) → sitemap discovery → `httpx` с per-domain rate-limit (0.5 req/s) и Cloudflare-detect → BeautifulSoup/Schema.org/Tilda JSON → `ListingDTO` → UPSERT в БД → отдельный matcher идёт в Discogs API по barcode/catalog для создания недостающих `records`.

**Cron-задачи**: incremental crawl каждую ночь (~10 мин на магазин), full re-crawl раз в неделю, stock-refresh каждые 6 часов (для активных листингов в карусели), matcher batch каждый час.
...
📖 **Детальная операционка**: [docs/PARSING.md](docs/PARSING.md) — архитектура, cron, лимиты, ресурсы, HOWTO добавить новый магазин, troubleshooting.
